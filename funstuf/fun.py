...
class FashionMNISTDataset(Dataset):
    """Fashion MNIST dataset loaded from idx files"""
    def __init__(self, data_dir, split="train", V=1):
        self.V = V
        ...
        # Augmentations for training - adapted for grayscale/simpler images
        self.aug = v2.Compose([
            v2.ToImage(),
            v2.Grayscale(num_output_channels=3),  # Convert to 3-channel
            v2.Resize(128),
    ...
    def __getitem__(self, i):
        img = self.images[i]  # (28, 28) uint8
        label = int(self.labels[i])
        transform = self.aug if self.V > 1 else self.test
        views = torch.stack([transform(img) for _ in range(self.V)])
        return views, label
...
def main(cfg: DictConfig):
    wandb.init(project="LeJEPA-FashionMNIST", config=dict(cfg))
    torch.manual_seed(0)

    # Use Fashion MNIST dataset
    data_dir = Path("./datasets/fmnist")
    train_ds = FashionMNISTDataset(data_dir, split="train", V=cfg.V)
    test_ds = FashionMNISTDataset(data_dir, split="test", V=1)
    train = DataLoader(
